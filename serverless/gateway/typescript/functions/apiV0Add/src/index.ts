...
export const stripBasePath = (files: InMemoryFile[]) => {
  ...
  return files
    .map((file) => ({
      path: basePath ? path.relative(basePath, file.path + "/") : file.path,
...
export const apiV0Add = async (event: any, context: any) => {
  if (!WRAPPERS_BUCKET) {
    return internalServerError(
      "Error: Environment variable WRAPPERS_BUCKET missing"
    );
  }

  if (!UPLOADED_WRAPPERS_TABLE) {
    return internalServerError(
      "Error: Environment variable UPLOADED_WRAPPERS_TABLE missing"
    );
  }
  ...
  if (!result.valid) {
    return internalServerError(
      `Upload is not a valid wrapper. Reason: ${result.failReason}`
    );
  }

...
const ControllersPage = (): ReactElement<any, any> => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const groupedWrappers = groupBy(wrappersWithENS, x => `${x.owner}-${x.network}`);
      ...
      const allControllers = Object.keys(groupedWrappers).map(key => {
        ...
        return {
          ...
          downloadCount: groupedWrappers[key].reduce((acc, wrapper) => acc + wrapper.downloadCount, 0)
        };
      });

      const sortedControllers = allControllers.sort(
        (a: any, b: any) => b.wrapperCount - a.wrapperCount
      ).sort(
        (a: any, b: any) => a.network > b.network ? 1 : a.network < b.network ? -1 : 0
      );
          ...
          <table className="table" cellSpacing="3" cellPadding="3">
            ...
            <tbody>
              ...
              <Link key={index} href={`/controllers/${controller.address}`}>
                <tr key={index}>
                  <td>
                    <span>{controller.name ?? toPrettyHex(controller.address)}</span>
                  </td>
                  <td>

...
export const populateEnsDomainOwners = async (wrappers: WrapperEnsModel[], registry: Contract, provider: Provider, network: string): Promise<DetailedWrapperEnsModel[]> => {
  ...
  const detailedWrappers: DetailedWrapperEnsModel[] = await mapWithMultiCall(
    ...
    provider, 
    async (wrapper, call) => {
      const owner = await domainOwnersCache.getOrUpdate(
        wrapper.ens.node, 
        () => call<string>(
          registry.owner(ethers.utils.hexlify(wrapper.ens.node))
        )
      ) as string;

      const ownerDomain = owner 
        ? await reverseLookupCache.getOrUpdate(
          wrapper.ens.node, 
          async () => {
            const resolverAddress = await call<string>(
              registry.resolver(ethers.utils.hexlify(ethers.utils.namehash(`${owner.slice(2)}.addr.reverse`)))
            );
            ...
            const ownerDomain = await call<string>(
              resolverContract.name(ethers.utils.hexlify(ethers.utils.namehash(`${owner.slice(2)}.addr.reverse`)))
            );
